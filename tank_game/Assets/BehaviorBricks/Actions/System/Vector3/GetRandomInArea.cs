...
namespace BBUnity.Actions
{
    ...
    public class GetRandomInArea : GOAction
    {
        ...
        public override void OnStart()
        {
            ...
            if (boxCollider != null)
            {
                randomPosition = new Vector3(UnityEngine.Random.Range(area.transform.position.x - area.transform.localScale.x * boxCollider.size.x * 1.5f,
                                                                      area.transform.position.x + area.transform.localScale.x * boxCollider.size.x * 1.5f),
                                             area.transform.position.y,
                                             UnityEngine.Random.Range(area.transform.position.z - area.transform.localScale.z * boxCollider.size.z * 1.5f,
                                                                      area.transform.position.z + area.transform.localScale.z * boxCollider.size.z * 1.5f));
            }
            else
            {
